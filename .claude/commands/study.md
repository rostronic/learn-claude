...
4. **Walk the user through it — don't dump the markdown.** Follow the lesson's actual section headings in order (the current template is Overview → How it works → Worked example → Anti-patterns & pitfalls → Exam focus *(optional)* → References → Exam coverage *(footer)*, but read the file rather than assuming — lessons are exam-agnostic, so some omit the exam sections). For each section:
...
6. **If the lesson has an "Exam focus" / "Exam coverage" section,** be concrete about which exam(s) it maps to (e.g. CCAF) and which of that exam's scenarios it powers. If the lesson is unmapped, there's nothing to surface here — that's expected.
