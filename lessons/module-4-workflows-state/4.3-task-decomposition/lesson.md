...
When a job genuinely needs more than a handful of subagents — "dozens to hundreds of agents" — that's the boundary where turn-by-turn delegation gives way to the **`Workflow` tool / dynamic workflows**, which "moves the orchestration into a script the runtime executes outside the conversation context" ([Agent SDK subagents](https://code.claude.com/docs/en/agent-sdk/subagents)). How that script-orchestrated model differs from turn-by-turn delegation is its own topic ([Agent SDK dynamic workflows](https://code.claude.com/docs/en/workflows)). For the decomposition sizes this lesson covers, plain subagent delegation is the right tool; know the escalation exists.
